...
def scan(time_now):
    ...
    try:
        ...
        #                         bottomRight = item[1]
        #                         file.write(f'{topLeft[0]},{topLeft[1]}, {bottomRight[0]},{bottomRight[1]}\n')
        img = cv2.imread(f"{time_now}.jpg")

        client.publish("mqtt/mes/wavesoldering", "Done")
        busy = False

        if img is None:
            show_message_OLED("No captured image")
            return
        # cv2.imwrite(
        ...
        if gray_image is None:
            show_message_OLED("gray convert fail")
            return
        cv2.imwrite(f"images/{time_now}.jpg", gray_image)
        image = img.copy()
        try:
            # with open(f"coordinate-0,0,1,1.txt", "r") as file:
            #     for line in file:
            #         top_left_x: int = int(line.strip().split(",")[0])
            #         top_left_y: int = int(line.strip().split(",")[1])
            #         bottom_right_x: int = int(line.strip().split(",")[2])
            ...
            #     topLeft[1] : bottomRight[1], topLeft[0] : bottomRight[0]
            # ]
            partial_line = gray_image

            cv2.imwrite(f"partial.jpg", partial_line)
            # cv2.imwrite(
            #     f'images/{strftime("%Y-%m-%d %H:%M:%S", gmtime())}.jpg',
            #     partial_line,
            # )
            pil_image = Image.open(f"partial.jpg")
            ...
            pcb_data = []
            gets = None
            with Image.open(f"images/{time_now}.jpg") as capture_image:
                buffered = BytesIO()
            ...
            try:
                api_url = 'http://10.100.27.25:8080/api/detectbarcode'
            ...
            if panel_data is None:
                show_message_OLED("No panel code")
                return
            ...
            pcb_code = ""
            if pcb_data is None:
                show_message_OLED("No PCB code")
                return
            for item in pcb_data:
                pcb_code = pcb_code + item + ";"
            if pcb_code.endswith(";"):
                pcb_code = pcb_code[:-1]
            print(pcb_code)
            base_url = (
                "http://fvn-s-ws01.friwo.local:8088/api/ProcessLock/FA/InsertLinkPanel/"
            )
            endpoint = f"{panel_code}/{pcb_code}"
            ...
            show_message_OLED(f"Exception: {e}")
            print(e)
            # topLeft = (0, 0)
...
def on_message(client, userdata, msg):
    ...
    if not busy:
        ...
        if str(msg.payload) == "b'Ready to scan'":
            ...
            time.sleep(0)
            for area in areas:
                capture_frame(False, area, f"{time_now}.jpg")
